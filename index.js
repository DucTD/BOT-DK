...
if (i.customId.startsWith("approve_")) {
  ...
  // Cập nhật DB + reset awaitingBill + lastBill
  ...
  if (guild) {
    ...
    if (member) {
      ...
      // ✅ DM thông báo approve
      await member.send(`🎉 Gói VIP ${data.plan} của bạn đã được duyệt! Hạn đến: <t:${Math.floor(expire/1000)}:F>`).catch(() => {});
    }
  }
